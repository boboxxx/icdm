"""Locked AWGN validation of point vs joint-moment calibration; no training."""
import argparse
import hashlib
import json
import math
...
def receive(mode,sampler,decoder,y,h,snr,block_size,device):
    if mode in {'NO_ICDM','A3','A4'}:
        return recover(mode,sampler,decoder,y,h,snr,None,'awgn',block_size,device)
    sync(device)
    t0=time.perf_counter()
    latent,_,estimates=sampler.SIC_sampling_alternating(snr,y,h,'awgn',block_size,variant=mode)
    sync(device)
    ...
    if not torch.isfinite(decoded).all(): raise FloatingPointError('Nonfinite decoded output')
    return decoded,estimates,{'sampling_and_calibration_seconds':t1-t0,
