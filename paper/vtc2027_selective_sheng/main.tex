...
decision. A separate no-fine-tuning diagnostic uses their public checkpoints;
we label its training-domain and bandwidth mismatches and exclude it from
matched rankings.
...
checkpoint, while MambaJSCC uses its official repository. For reproducibility,
we additionally evaluate repository-released weights without fine-tuning on the
same confirmation conditions. The public MambaJSCC variants retain CBR $1/48$
but differ in training data, channel and resolution; the available public
DeepJSCC weight has CBR $0.16256$ and is therefore a higher-bandwidth diagnostic.
These results are archived separately and are not used for superiority claims.
