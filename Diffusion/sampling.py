...
from Diffusion.utils import from_flattened_numpy, to_flattened_numpy, get_score_fn, get_noise_fn
from scipy import integrate
from Diffusion import sde_lib as sde_lib
import Diffusion.utils as mutils
from Diffusion.uni_pc import NoiseScheduleVP, model_wrapper, UniPC, Joint_UniPC
...
def get_sampling_fn(config, sde, shape, inverse_scaler, eps, sampler_name="uni_pc"):
    ...
    elif sampler_name.lower() == "joint_unipc":
        # print("order:", 3)
        sampling_fn = get_joint_uni_pc_sampler(
            sde=sde,
            shape=shape,
            inverse_scaler=inverse_scaler,
            steps=40,
...
def get_uni_pc_sampler(
    ...
    def uni_pc_sampler(model, A=None, Ap=None, deg_feature=None):
...
def get_joint_uni_pc_sampler(
    ...
    def joint_uni_pc_sampler(
        ...
        with torch.no_grad():
            noise_pred_fn_x = get_noise_fn(
                sde, model_x, train=False, continuous=True
            )  # 这里有一个0-1与0-999对其问题
            noise_pred_fn_n = get_noise_fn(sde, model_n, train=False, continuous=True)
            joint_uni_pc = Joint_UniPC(
                noise_pred_fn_x,
                noise_pred_fn_n,
                ns,
                algorithm_type=algorithm_type,
            ...
            return inverse_scaler(x), inverse_scaler(n)

    return joint_uni_pc_sampler
