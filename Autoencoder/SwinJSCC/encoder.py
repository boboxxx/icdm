from Autoencoder.SwinJSCC.modules import *
import torch
import math
...
class SwinJSCC_Encoder(nn.Module):
    ...
    def forward(self, x):
        ...
        B, S, C = x.shape
        # print(B,S,C)
        x = x.permute(0, 2, 1).reshape(B, C, H // 2**self.num_layers, W // 2**self.num_layers)
        # print(x.numel()/num)
        x = self.head_list(x)
...
def create_SwinJSCC_encoder(config):
    model = SwinJSCC_Encoder(
        # img_size=(32, 32),
