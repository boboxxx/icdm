...
from Autoencoder.SwinJSCC.encoder import SwinTransformerBlock, AdaptiveModulator
...
def create_SwinJSCC_decoder(config):

    model = SwinJSCC_Decoder(
        # img_size=(32, 32),
...
def build_model(config):
    ...
    with torch.no_grad():
        ...
        delta_t = t1 - t0
        print(
            "Decoding Time per img {}s".format(
                (delta_t.seconds + 1e-6 * delta_t.microseconds) / 100
            )
        )
