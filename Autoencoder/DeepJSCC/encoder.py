from Autoencoder.DeepJSCC.modules import *
import torch
import math
...
class SwinTransformerBlock(nn.Module):
    def __init__(self, dim, input_resolution, num_heads, window_size=7, shift_size=0, mlp_ratio=4.0, qkv_bias=True, qk_scale=None, act_layer=nn.GELU, norm_layer=nn.LayerNorm):
        ...
        self.attn = WindowAttention(dim, window_size=to_2tuple(self.window_size), num_heads=num_heads, qkv_bias=qkv_bias, qk_scale=qk_scale)
...
class AdaptiveModulator(nn.Module):
    def __init__(self, M):
        ...
        self.fc = nn.Sequential(nn.Linear(1, M), nn.ReLU(), nn.Linear(M, M), nn.ReLU(), nn.Linear(M, M), nn.Sigmoid())
...
class JSCC_Encoder(nn.Module):
    def __init__(
        ...
        self.head_list = torch.nn.Conv2d(embed_dims[-1], C, kernel_size=1, padding=0, stride=1)
    ...
    def forward(self, x):
        ...
        B, S, C = x.shape
        # print(B,S,C)
        x = x.permute(0, 2, 1).reshape(B, C, int(math.sqrt(S)), int(math.sqrt(S)))
        # print(x.numel()/num)
        x = self.head_list(x)
        # x = DiagonalGaussianDistribution(x)
        # x=torch.nn.DataParallel(x,device_ids=[0])
        return x
...
def create_DeepJSCC_encoder(**kwargs):
    model = JSCC_Encoder(**kwargs)
